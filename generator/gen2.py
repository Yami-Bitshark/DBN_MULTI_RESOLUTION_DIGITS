...
for i in range(2,11):
    ...
    cv2.imwrite(fold+"/"+str(sys.argv[2])+"_"+str(tup1(i)[1])+"x"+str(tup1(i)[0])+".png",y)
    #print "{}x{}".format(tup2(i),i)
    y=cv2.resize(img,tup2(i),interpolation=cv2.INTER_CUBIC)
    y=cv2.equalizeHist(y)
    cv2.imwrite(fold+"/"+str(sys.argv[2])+"_"+str(tup2(i)[1])+"x"+str(tup2(i)[0])+".png",y)
    #print "{}x{}".format(tup3(i),i)
    y=cv2.resize(img,tup3(i),interpolation=cv2.INTER_CUBIC)
    y=cv2.equalizeHist(y)
    cv2.imwrite(fold+"/"+str(sys.argv[2])+"_"+str(tup3(i)[1])+"x"+str(tup3(i)[0])+".png",y)
    #print "{}x{}".format(tup4(i),i)
    y=cv2.resize(img,tup4(i),interpolation=cv2.INTER_CUBIC)
    y=cv2.equalizeHist(y)
    cv2.imwrite(fold+"/"+str(sys.argv[2])+"_"+str(tup4(i)[1])+"x"+str(tup4(i)[0])+".png",y)
    #print "{}x{}".format(tup5(i),i)
    y=cv2.resize(img,tup5(i),interpolation=cv2.INTER_CUBIC)
    y=cv2.equalizeHist(y)
    cv2.imwrite(fold+"/"+str(sys.argv[2])+"_"+str(tup5(i)[1])+"x"+str(tup5(i)[0])+".png",y)
